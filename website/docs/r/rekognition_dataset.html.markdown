...
```terraform
import {
  to = aws_rekognition_dataset.example
  identity = {
<!---
...
* `example_id_arg` - ID argument of the Dataset.
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import Rekognition Dataset using the `example_id_arg`. For example:

```terraform
import {
  to = aws_rekognition_dataset.example
  id = "dataset-id-12345678"
}
```

Using `terraform import`, import Rekognition Dataset using the `example_id_arg`. For example:

```console
% terraform import aws_rekognition_dataset.example dataset-id-12345678
```
